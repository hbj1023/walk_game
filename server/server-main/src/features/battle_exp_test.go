...
func TestRecommendedCombatPowerForStage(t *testing.T) {
	...
	if got := recommendedCombatPowerForStage(6); got != 420 {
		t.Fatalf("stage 6 recommended power = %d, want 420", got)
	}
	if got := recommendedCombatPowerForStage(10); got != 780 {
		t.Fatalf("stage 10 recommended power = %d, want 780", got)
	}
	if got := recommendedCombatPowerForStage(11); got != 780 {
		t.Fatalf("stage 11 recommended power = %d, want 780", got)
	}
	if got := recommendedCombatPowerForStage(13); got != 920 {
		t.Fatalf("stage 13 recommended power = %d, want 920", got)
	}
	if got := recommendedCombatPowerForStage(15); got != 1200 {
		t.Fatalf("stage 15 recommended power = %d, want 1200", got)
	}
}
